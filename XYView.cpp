...
void XYView::setX( int l, int p, double xx )
{
  if ( l < MAXLINES ) {
    if ( p < MAXPOINTS ) {
      x[l][p] = xx;
...
void XYView::setY( int l, int p, double yy )
{
  if ( l < MAXLINES ) {
    if ( p < MAXPOINTS ) {
      y[l][p] = yy;
...
// リングバッファへのデータ追加
void XYView::NewPointR( int tt, double yy0, double yy1, double yy2 )
{
  mont[ points[0] ] = tt;
  mony[0][ points[0] ] = yy0;
  mony[1][ points[0] ] = yy1;
...
void XYView::SetLines( int Lines )
{
  lines = Lines;
}

void XYView::DrawMonitor( QPainter *p )
{
  QString buf, buf2;
...
void XYView::UpDateYWindowRing( void )
{
...
void XYView::SetMonScale( int ms )
{
  MonScale = ms;
}

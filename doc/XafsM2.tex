...
\FIG{b}{
  \hfil\scalebox{\TWpt}{\scalebox{0.35}{\includegraphics{OutLook1.png}}}
  \caption{XafsM2の概観}\label{XafsM2の概観}
}

2段目は、グループ分けされた操作がまとまったタブになっていて、
その時時に必要なタブを選択することになります。
...
\FIG{t}{
  \hfil\scalebox{\TWpt}{\scalebox{0.50}{\includegraphics{ElementSelect.png}}}
  \caption{測定対象の元素選択部分}\label{元素選択部}
}

\FIG{t}{
  \MINI{8cm}{
    \hfil\scalebox{\TWpt}{\scalebox{0.50}{\includegraphics{ElemSelectBox.png}}}
    \caption{測定元素選択ボックスと元素一覧表}\label{元素選択ボックス}
  }
  \MINI{8cm}{
    \hfil\scalebox{\TWpt}{\scalebox{0.30}{\includegraphics{PeriodicTable.png}}}
    \caption{周期表を使った元素選択。
